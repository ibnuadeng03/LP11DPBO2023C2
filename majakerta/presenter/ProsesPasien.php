	...
	function prosesDataPasien()
	{
		try {
			...
			while ($row = $this->tabelpasien->getResult()) {
				...
				$pasien->setTempat($row['tempat']); 			//mengisi tempat lahir
				$pasien->setTl($row['tl']); 					//mengisi tanggal lahir
				$pasien->setGender($row['gender']); 			//mengisi jenis kelamin
				$pasien->setEmail($row['email']);				//mengisi surel
				$pasien->setTelp($row['telp']);					//mengisi no telepon
